...
describe("getEventActivityMessageKey", () => {
  it("maps active and historical events to their message keys", () => {
    expect(getEventActivityMessageKey(true, locale)).toBe(
      "event_status_active",
    );
    expect(getEventActivityMessageKey(false, locale)).toBe(
      "event_status_historical",
    );
  });
});
...
describe("getHomepageEventStatusMessageKey", () => {
  ...
  it("reports no upcoming date without a next edition", () => {
    expect(getHomepageEventStatusMessageKey(undefined, today, locale)).toBe(
  ...
  it("reports an in-progress edition that starts today", () => {
    expect(
      getHomepageEventStatusMessageKey({ startDate: today }, today, locale),
    ).toBe("event_status_in_progress");
  });
...
describe("getRegistrationPresentation", () => {
  it("falls back to unavailable without a status and without a URL", () => {
    expect(getRegistrationPresentation(undefined, undefined, locale)).toEqual({
      key: "event_registration_unavailable",
    });
  });

  it("maps each registration status to its message key without a URL", () => {
    expect(getRegistrationPresentation("open", undefined, locale)).toEqual({
      key: "event_registration_open",
    });
    expect(getRegistrationPresentation("closed", undefined, locale)).toEqual({
      key: "event_registration_closed",
    });
    expect(
      getRegistrationPresentation("coming-soon", undefined, locale),
    ).toEqual({
      key: "event_registration_coming_soon",
    });
    expect(
      getRegistrationPresentation("unavailable", undefined, locale),
    ).toEqual({
      key: "event_registration_unavailable",
    });
  });
